...
def relative_camera_motion_metric(
    ...
    the metre-per-VGGT-unit scale recovered from physical/predicted camera
    height. The learned Scale Token is not used for this translation.
    """

    if not math.isfinite(metric_scale_m_per_vggt) or metric_scale_m_per_vggt <= 0:
        raise VerifierPlanningError(
    ...
    transform[:3, 3] = relative_translation_vggt * metric_scale_m_per_vggt
    return transform
