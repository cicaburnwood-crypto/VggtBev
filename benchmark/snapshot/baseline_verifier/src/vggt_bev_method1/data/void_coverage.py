...
class VoidCoverageIndex:
    """Render loss-only valid-geometry masks from immutable scene sidecars."""
        ...
        *,
        ...
        if require_complete:
            if bool(payload.get("partial", True)):
                raise ValueError("training requires a complete Void coverage index")
            if int(payload.get("missing_floor_band_count", -1)) != 0:
                raise ValueError("training Void coverage has missing floor bands")
