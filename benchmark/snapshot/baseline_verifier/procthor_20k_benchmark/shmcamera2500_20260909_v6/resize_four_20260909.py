...
AUDIT = ROOT / 'operations' / 'resize_6to4_20260909'
...
def stop():
    ...
    for lane in range(6):
        ...
        if (process.stat().st_uid != os.getuid() or
                argv != ['bash', str(HERE / 'launch.sh'), str(lane), str(lane)]):
    ...
    save(AUDIT / 'STOP_REQUEST.json', dict(utc=now(), reason='User requested four GPUs', targets=targets))
    for item in targets:
...
def resume():
    ...
    records = []
    for _ in range(10):
        raw = command(['nvidia-smi', '-i', '0,1,2,3',
    ...
    gpus = records[-1]['rows']
    if len(gpus) != 4 or any(g['uuid'] in busy for g in gpus) or any(
            row['memory_percent'] >= 30 or row['compute_percent'] >= 20
            for sample in records for row in sample['rows']):
        raise RuntimeError('Four requested retained GPUs are not idle; no restart')
    save(AUDIT / 'RESUME_PREFLIGHT.json', dict(samples=records, compute_apps=apps))
    allocation = dict(utc=now(), operation='user_authorized_resize_6_to_4',
                      lanes_count=4, groups_total=500, original_started_utc='2026-09-09T12:10:24.099852+00:00',
                      assignment='group_number modulo 4', lanes=[], immutable_baseline=str(AUDIT/'STOPPED.json'))
    for gpu in gpus:
        lane = gpu['index']
        name = f'shm_v6_gpu{lane}_20260909_4gpu'
        ...
        allocation['lanes'].append(dict(lane=lane, gpu_index=lane, uuid=gpu['uuid'], tmux=name,
                                       command=cmd, assigned_groups=125))
