"""Paper-level evaluation for the frozen WTBD implicit Merged+Scale head.
...
from vggt_bev_method1.cli_train_wtbd import (
    _bev_weights,
    _contract,
    _sha256,
    build_model,
...
from vggt_bev_method1.wtbd_train_utils import build_wtbd_datasets, wtbd_collate
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate WTBD on the complete unseen split")
...
def main() -> None:
    args = parse_args()
    config = load_wtbd_config(args.config)
    training = config["training"]
    seed_everything(int(training["seed"]))
    distributed, rank, world_size, _, device, preflight = distributed_runtime(training)
    if not distributed:
        raise RuntimeError("WTBD full evaluation requires torchrun")
    _, validation = build_wtbd_datasets(config, verify_manifest=False)
    ...
    with shard_path.open("w", encoding="utf-8") as stream:
        for batch in loader:
            ...
            bev_loss = p1b_bev_loss(
                ...
                weights=_bev_weights(training), wrong_evidence_scale=1.0,
