...
def wtbd_collate(samples: list[dict]) -> dict:
    if not samples:
        raise ValueError("cannot collate an empty batch")
    frame_counts = {int(sample["images"].shape[0]) for sample in samples}
    if len(frame_counts) != 1:
        raise ValueError("one batch must contain equal-length RGB histories")
    tensor_keys = (
        "images",
        "scale_gt_depth_m",
        "scale_gt_valid_mask",
    ...
    if any(key.startswith("single_") for key in output):
        raise RuntimeError("WTBD collate exposed a Single target")
    return output


def build_wtbd_datasets(
    config: dict,
    *,
