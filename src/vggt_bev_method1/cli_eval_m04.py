"""Unseen-scene evaluation for the non-recurrent M04 Merged+Scale model."""
...
from vggt_bev_method1.cli_train_m04 import _contract, _sha256, build_model
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate M04 on one final RGB prefix per unseen session"
    )
...
def _load_head(path: Path, model, contract: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, expected in contract.items():
        if state.get(key) != expected:
            raise ValueError(f"M04 evaluation contract mismatch for {key}")
...
def main() -> None:
    ...
    _, validation = build_m04_datasets(
        config,
    ...
    state = _load_head(arguments.checkpoint.expanduser().resolve(), model, contract)
    model.eval()
    ...
    with shard_path.open("w", encoding="utf-8") as stream:
        for batch in loader:
            ...
            for sample_index, (item, meta) in enumerate(
                zip(sample_values, metadata, strict=True)
            ):
                item.update(
                    {
                        ...
                        "source_coverage_fraction": float(
                            targets["source_coverage_fraction"][sample_index].item()
                        ),
                        "effective_supervision_fraction": float(
                            targets["gt_valid_mask"][sample_index]
                            .float()
                            .mean()
                            .item()
                        ),
                        "effective_metric_extent_gt": float(
                            targets["effective_metric_extent_gt"][sample_index].item()
                        ),
            ...
            if (
                processed % arguments.log_every < len(metadata)
                and rank == 0
            ):
    ...
    if rank == 0:
        ...
        valid_scale_coverages = [
            record["source_coverage_fraction"]
            for record in records
            if record.get("scale_valid", False)
        ]
        summary.update(
            {
                "evaluation_contract": (
                ...
                "source_coverage_mean": sum(
                    record["source_coverage_fraction"] for record in records
                )
                / max(len(records), 1),
                "valid_scale_source_coverage_mean": sum(
                    valid_scale_coverages
                )
                / max(len(valid_scale_coverages), 1),
                "effective_supervision_mean": sum(
                    record["effective_supervision_fraction"]
                    for record in records
                )
                / max(len(records), 1),
            }
