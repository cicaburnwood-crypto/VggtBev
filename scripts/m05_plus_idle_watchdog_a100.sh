...
resume_script="$code/scripts/resume_m05_plus_step70000_a100.sh"
resume_log="$code/logs/m05_plus_watchdog_resume_step70000_20260907.log"
train_tmux=m05_plus_a100_resume_step70000_watchdog
...
idle_count=0
if [[ -f "$state" ]]; then
    saved_count=$(awk -F= '$1=="idle_count" {print $2}' "$state" | tail -n 1)
    if [[ "$saved_count" =~ ^[0-9]+$ ]]; then
        idle_count=$saved_count
    fi
fi
