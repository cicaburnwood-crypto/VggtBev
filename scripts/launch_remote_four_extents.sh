...
run_tag="${1:?usage: launch_remote_four_extents.sh RUN_TAG}"
mkdir -p "$project_root/logs"

gpus=(1 2 3 4)
extents=(3p5m 5m 6p5m 8m)

for index in "${!gpus[@]}"; do
    gpu="${gpus[$index]}"
    extent="${extents[$index]}"
    session="vggtbev_m2_${extent}_g${gpu}_${run_tag}"
    log="$project_root/logs/method2_bev_${extent}_gpu${gpu}_${run_tag}.log"
    config="$project_root/configs/remote_hm3d_method2_bev_${extent}.toml"
