...
if [[ ! -f "$session_cache" ]]; then
    echo "required session-record cache is missing: $session_cache" >&2
    exit 1
fi
...
echo "m05pp_training_launch manifest=$manifest manifest_sha256=$manifest_sha256"
exec "$python_bin" -m torch.distributed.run \
    ...
    --master_port=29506 \
    -m vggt_bev_method1.cli_train_m05_pp \
    --config configs/m05_pp_a100_8gpu_10e_frozen.toml
