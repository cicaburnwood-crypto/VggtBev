...
for session in "${sessions[@]}"; do
    ...
    cp "${session}/COMPLETE" "${session}/metadata.json" "${destination}/"
    cp -a "${session}/camera" "${destination}/camera"
    cp -a "${session}/depth" "${destination}/depth"
    cp -a "${session}/bev_6p5m/masked" \
        "${destination}/bev_6p5m/masked"
    cp -a "${session}/bev_6p5m/merged_masked_10m" \
        "${destination}/bev_6p5m/merged_masked_10m"
done
